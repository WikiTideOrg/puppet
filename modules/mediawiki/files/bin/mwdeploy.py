...
repos = {**versions, 'config': 'config', 'errorpages': 'ErrorPages'}
...
def get_extensions_in_pack(pack_name: str) -> list[str]:
    packs = {
        ...
        'wikitide': ['CreateWiki', 'DataDump', 'GlobalNewFiles', 'ImportDump', 'IncidentReporting', 'ManageWiki', 'MatomoAnalytics', 'PDFEmbed', 'RemovePII', 'RottenLinks', 'SpriteSheet', 'WikiDiscover', 'WikiTideMagic', 'YouTube'],
    }
...
def run_process(args: argparse.Namespace, start: float, version: str = '') -> None:  # pragma: no cover
    ...
    options = {'config': args.config and not version, 'world': args.world and version, 'errorpages': args.errorpages and not version}
    exitcodes = []
    loginfo = {}
    rsyncpaths = []
    rsyncfiles = []
