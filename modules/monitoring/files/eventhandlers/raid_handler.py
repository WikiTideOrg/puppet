...
RAID_TYPES = ('megacli', 'perccli', 'n/a')
COMPRESSED_RAID_TYPES = ('megacli')
...
PHORGE_TAG = 'Site Reliability Engineering'
...
PHORGE_TASK_DESCRIPTION_PREFIX = (
    ...
    "Please **contact the datacenter** to coordinate a replacement."
)
...
def get_raid_status(host, raid_type):
    ...
    if raid_type in COMPRESSED_RAID_TYPES:
        ...
        try:
            status = zlib.decompress(stdout.replace(b'###NULL###', b'\x00')).decode('utf-8')
...
def get_phorge_client():
    """Return a Phroge client instance"""

    parser = configparser.SafeConfigParser()
    parser_mode = 'phorge_bot'
    parser.read(PHORGE_CONFIG_FILE)
...
def get_phorge_project_ids(phorge_client):
    ...
    project_name = PHORGE_TAG
    projects = phorge_client.project.query(names=[project_name])

    if len(projects.data.keys()) != 1:
...
def open_phorge_task(
        phorge_client, project_ids, host, raid_type, raid_status, icinga_url):
    """ Open a task on Phorge and return it
    ...
    description_prefix = PHORGE_TASK_DESCRIPTION_PREFIX.format(
        type=raid_type, host=host, url=icinga_url)
    description = '{description_prefix}\n```\n{raid_status}\n```'.format(
        description_prefix=description_prefix, raid_status=raid_status)

    task = phorge_client.maniphest.edit(transactions=[
        {'type': 'title', 'value': PHORGE_TASK_TITLE.format(host=host)},
        {'type': 'description', 'value': description},
        {'type': 'projects.set', 'value': project_ids},
    ])
    task_id = task.object['id']

    logger.debug('Opened Phorge task: {}'.format(task_id))
    return task_id
...
def acknowledge_nagios_alert(host, service_description, task_id):
    """ Acknowledge the Nagios/Icinga alert
        ...
        task_id             -- the ID of the related Phorge task
    """
