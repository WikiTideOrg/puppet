...
# Adapted for WikiForge by Universal Omega 2023
...
def backup_mediawiki_static(dt: str, database: str):
    ...
    excluded_dirs = {'archive', 'deleted', 'dumps', 'lockdir', 'sitemaps', 'temp', 'thumb'}

    for db in dbs:
        with SevenZipFile(f'/mnt/mediawiki-static/{db}.7z', mode='w') as archive:
            for root, dirs, files in os.walk(f'/mnt/mediawiki-static/{db}'):
                dirs[:] = [d for d in dirs if d not in excluded_dirs]

                for file in files:
                    file_path = os.path.join(root, file)
                    archive_path = os.path.relpath(file_path, f'/mnt/mediawiki-static/{db}')
...
def download(source: str, dt: str, database: str):
    ...
    elif source in ['sql', 'mediawiki-static']:
        download_pca(f'{source}/{database}/{dt}.7z')
